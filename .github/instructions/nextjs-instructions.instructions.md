...
# Industrial best practices (Next.js)

- **Server vs client boundary:** Keep server-only work (data fetching, auth checks) in Server Components or route handlers; push interactivity into Client Components. Avoid mixing boundaries in one file.
- **Data fetching:** Prefer direct server-side calls (services/lib) over calling your own API routes from Server Components to avoid extra hops and latency.
- **Caching:** Use Cache Components and tags intentionally; document revalidation points (e.g. after which mutations) so cache behavior stays predictable in production.
- **Async request APIs:** In Next.js 16+, always `await` `cookies()`, `headers()`, `params`, and `searchParams` in Server Components and route handlers to avoid subtle bugs.
- **Security:** Validate and sanitize all input in API routes; enforce auth/authorization server-side; never trust client-only checks for sensitive operations.
