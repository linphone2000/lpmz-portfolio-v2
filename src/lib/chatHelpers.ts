...
export function getFallbackResponse(question: string): string | null {
  ...
  if (
    lowerQuestion.match(
      /(what.*projects|tell.*projects|show.*projects|list.*projects|projects.*built|projects.*worked)/
    )
  ) {
  ...
  if (
    lowerQuestion.match(
      /(what.*skills|what.*technologies|what.*tech|what.*languages|what.*do you know|what.*can you do)/
    )
  ) {
    const mainSkills = [
  ...
  if (
    lowerQuestion.match(
      /(what.*experience|where.*worked|tell.*experience|what.*job|what.*companies)/
    )
  ) {
  ...
  if (
    lowerQuestion.match(
      /(where.*study|what.*education|what.*degree|where.*graduate|what.*university|what.*school)/
    )
  ) {
  ...
  // Location questions
  if (
    lowerQuestion.match(/(where.*from|where.*located|where.*live|where.*based)/)
  ) {
  ...
  if (
    lowerQuestion.match(
      /(what.*can.*you|how.*can.*you.*help|what.*help|what.*do)/
    )
  ) {
